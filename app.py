...
app.layout = html.Div([
    html.H1('Corona Virus Pandemic ~ by Tech Sarwesh😀', style={'color': '#fff', 'text-align': 'center'}),
    html.Div([
        html.Div([
            html.Div([
                html.Div([
...
def generate_graph(type):
    ...
    if type == 'Mask':
        return {'data': [go.Line(x=patients['State'], y=patients['Mask'])],
                'layout': go.Layout(title='Commodities Total Count', plot_bgcolor='pink')
                }
    if type == 'Sanitizer':
        return {'data': [go.Bar(x=patients['State'], y=patients['Sanitizer'])],
                'layout': go.Layout(title='Commodities Total Count', plot_bgcolor='pink')
                }
    if type == 'Oxygen':
        return {'data': [go.Line(x=patients['State'], y=patients['Oxygen'])],
                'layout': go.Layout(title='Commodities Total Count', plot_bgcolor='pink')
                }
